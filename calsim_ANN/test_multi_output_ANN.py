...
ann_name = ann_name = '_'.join(map(lambda x: abbrev_map[x.lower()],output_stations))
...
with sess.as_default():
    with sess.graph.as_default():
        ...
        MSE = np.mean(((y_predicted-y_norm)/y_slope)**2,axis=0)
        MAPE = np.mean(np.abs(y_predicted-y_norm)/y_norm,axis=0)
        for loc_id in range(len(output_stations)):
            print("%s MSE = %d"% (output_stations[loc_id],MSE[loc_id]))
            print("%s MAPE = %.2f%%"% (output_stations[loc_id],MAPE[loc_id]*100))

        results = pd.DataFrame(data=(y_predicted-y_bias)/y_slope,    # values
                               index=date,    # 1st column as index
                               columns=output_stations)  # 1st row as the column names
        ...
        real_data = pd.DataFrame(data=y_data,    # values
                                 index=date,    # 1st column as index
                                 columns=output_stations)  # 1st row as the column names
